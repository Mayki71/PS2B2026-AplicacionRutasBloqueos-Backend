...
export class ReportsService {
  ...
  async create(dto: CreateReportDto) {
    // Usa el id_usuario enviado por el frontend (tomado del localStorage),
    // o el hardcodeado como fallback temporal mientras no haya JWT guard
    const id_usuario = dto.id_usuario ?? TODO_USUARIO_HARDCODEADO;
    ...
    const { data: ubicacion, error: errorUbicacion } = await supabase
      ...
      .insert({
        ...
        latitud_fin: dto.latitud_fin ?? null,
        longitud_fin: dto.longitud_fin ?? null,
  ...
  async findAll() {
    const { data, error } = await supabase
      ...
      .select(`
        ...
        ubicaciones (
          id_ubicacion, latitud, longitud, latitud_fin, longitud_fin, direccion, referencia
        ),
        tipos_bloqueo (
  ...
  async findMisReportes(id_usuario: number) {
    //const uid = id_usuario ?? TODO_USUARIO_HARDCODEADO;
    ...
    const { data, error } = await supabase
      ...
      .select(`
        ...
        hora_inicio,
        fecha_creacion,
        ubicaciones ( latitud, longitud, latitud_fin, longitud_fin, direccion, referencia ),
  ...
  async votar(id_reporte: number, dto: CreateVoteDto, id_usuario: number) {
  ...
  async addComentario(id_reporte: number, dto: CreateCommentDto, id_usuario: number) {
    //const id_usuario = dto.id_usuario ?? TODO_USUARIO_HARDCODEADO;
  ...
  /** Eliminar reporte — solo si el id_usuario coincide con el dueño */
  async deleteReporte(id_reporte: number, id_usuario: number) {
    // Verificar propiedad
    ...
    if (!rep) throw new NotFoundException(`Reporte ${id_reporte} no encontrado`);
    if (rep.id_usuario !== id_usuario) {
      throw new BadRequestException('No tienes permiso para eliminar este reporte');
    }

    // Eliminar en cascada: comentarios → votos → imágenes → reporte → ubicación
    await supabase.from('comentarios').delete().eq('id_reporte', id_reporte);
    await supabase.from('votaciones').delete().eq('id_reporte', id_reporte);
    await supabase.from('imagenes_reportes').delete().eq('id_reporte', id_reporte);
    ...
    if (deleted?.id_ubicacion) {
      await supabase.from('ubicaciones').delete().eq('id_ubicacion', deleted.id_ubicacion);
    }
