...
let ReportsService = class ReportsService {
    ...
    async findMisReportes(id_usuario) {
        const { data, error } = await supabase_config_1.supabase
            .from('reportes')
            .select(`
        id_reporte,
    ...
    async votar(id_reporte, dto, id_usuario) {
    ...
    async addComentario(id_reporte, dto, id_usuario) {
    ...
    async deleteReporte(id_reporte, id_usuario) {
        const { data: rep } = await supabase_config_1.supabase
            .from('reportes')
            .select('id_usuario')
            .eq('id_reporte', id_reporte)
            .single();
        if (!rep)
            throw new common_1.NotFoundException(`Reporte ${id_reporte} no encontrado`);
        if (rep.id_usuario !== id_usuario) {
            throw new common_1.BadRequestException('No tienes permiso para eliminar este reporte');
        }
        await supabase_config_1.supabase.from('comentarios').delete().eq('id_reporte', id_reporte);
        await supabase_config_1.supabase.from('votaciones').delete().eq('id_reporte', id_reporte);
        await supabase_config_1.supabase.from('imagenes_reportes').delete().eq('id_reporte', id_reporte);
        const { data: deleted } = await supabase_config_1.supabase
            .from('reportes')
            .delete()
            .eq('id_reporte', id_reporte)
            .select('id_ubicacion')
        ...
        if (deleted?.id_ubicacion) {
            await supabase_config_1.supabase.from('ubicaciones').delete().eq('id_ubicacion', deleted.id_ubicacion);
        }
